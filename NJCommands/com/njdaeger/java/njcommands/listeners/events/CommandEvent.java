...
public class CommandEvent {
	...
	/*
	 * private AfkCommand afkCommand = new AfkCommand(); private MessageCommand
	 * messageCommand = new MessageCommand(); private ReplyCommand replyCommand
	 * = new ReplyCommand(); private BroadcastCommand broadcastCommand = new
	 * BroadcastCommand(); private VanishCommand vanishCommand = new
	 * VanishCommand();
	 */
	...
	public void whenSpied(PlayerCommandPreprocessEvent e) {
		...
		if (Core.getConf().isSpiedCommand(string[0].substring(1, string[0].length()))) {
			for (User user : Core.getSpyingUsers()) {
				user.sendMessage(ChatColor.RED + "[SocialSpy] " + ChatColor.GRAY + Core.getUser(e.getPlayer())
				.getNickname() + ": " + ChatColor.ITALIC + ChatColor.GRAY + e.getMessage());
			}
		}
	}
